...
const { EC2Client, DescribeInstancesCommand, DescribeImagesCommand } = require("@aws-sdk/client-ec2");
const { mockClient } = require('aws-sdk-client-mock');

jest.mock('../src/stack', () => ({
  getInstance() {
    return { fetchOutputs: function () { return {} } }
  }
}));
...
describe("WorkflowJob", () => {
  ...
  describe("findRunnerSpec", () => {
    ...
    it("returns the config runner spec if found", async () => {
      context.payload.workflow_job.labels = ["runs-on,runner=cheap-arm64,spot=false"];
      const workflowJob = new WorkflowJob(context);
    ...
    it("returns a custom config", async () => {
      context.payload.workflow_job.labels = ["runs-on,cpu=2,family=c7+m7,image=my-image,spot=false"];
      const workflowJob = new WorkflowJob(context);
      workflowJob.repoConfig = {};
      const spec = await workflowJob.findRunnerSpec()
      expect(spec).toStrictEqual({
  ...
  describe("findImageSpec", () => {
    ...
    it("returns a custom image spec", async () => {
      context.payload.workflow_job.labels = ["runs-on,image=my-custom-image,ami=ami-5678909"];
      const workflowJob = new WorkflowJob(context);
      workflowJob.repoConfig = {};
      workflowJob.runnerSpec = {};
      const spec = await workflowJob.findImageSpec()
    ...
    it("returns a the runner spec image if one given", async () => {
      context.payload.workflow_job.labels = ["runs-on,runner=runner-with-image"];
      const workflowJob = new WorkflowJob(context);
      workflowJob.repoConfig = repoConfig;
  ...
  describe("findSshSpec", () => {
    ...
    it("returns the list of admins from the list of collaborators if no config given", async () => {
      ...
      nock('https://api.github.com')
        .get('/repos/runs-on/test/collaborators?permission=admin&affiliation=all')
  ...
  describe("setup()", () => {
    ...
    it("[deprecated] finds the docker image", async () => {
      context.payload.workflow_job.labels = ["runs-on,image=ubuntu22-docker-arm64"]
      const workflowJob = new WorkflowJob(context);
      workflowJob.repoConfig = repoConfig;
      ...
      expect(instanceImage.preinstall).toMatch("curl -fsSL https://get.docker.com | sh")
    })

    it("finds the arm image and default runner types", async () => {
      context.payload.workflow_job.labels = ["runs-on,image=ubuntu22-full-arm64"]
      const workflowJob = new WorkflowJob(context);
      workflowJob.repoConfig = repoConfig;
      await workflowJob.setup();
      const instanceImage = workflowJob.instanceImage;
    ...
    it("return instance types in the correct order", async () => {
      context.payload.workflow_job.labels = ["runs-on,runner=4cpu-linux,family=c7a+r7i,cpu=8+4+2"]
      const workflowJob = new WorkflowJob(context);
      workflowJob.repoConfig = repoConfig;
      await workflowJob.setup();
      const instanceTypes = workflowJob.instanceTypes;
    ...
    it("can't find matching instance types for labels", async () => {
      context.payload.workflow_job.labels = ["runs-on,runner=4cpu-linux,family=t4g"]
      const workflowJob = new WorkflowJob(context);
      workflowJob.repoConfig = repoConfig;
    ...
    it("can't find matching instance image for labels", async () => {
      context.payload.workflow_job.labels = ["runs-on,runner=4cpu-linux,ami=ami-123"]
      const workflowJob = new WorkflowJob(context);
      workflowJob.repoConfig = repoConfig;
      await expect(workflowJob.setup()).rejects.toThrow(
